package Year2019.June.day09;

/**
 * @Data 19/6/11
 * @题目描述： 实现 strStr() 函数。
 * 给定一个 haystack 字符串和一个 needle 字符串，在 haystack 字符串中找出 needle 字符串出现的第一个位置 (从0开始)。如果不存在，则返回  -1。
 * @示例1： 输入: haystack = "hello", needle = "ll"
 * 输出: 2
 * @示例2： 输入: haystack = "aaaaa", needle = "bba"
 * 输出: -1
 */
public class Implement_strStr {
    ...
    public static void main(String[] args) {
        String a = "ll";
        String b= "ll";
        System.out.println(new Implement_strStr().strStr(a, b));
    }
    public int strStr(String haystack, String needle) {
        ...
        for (int i = 0; i < array.length; i++) {
            if (array[i] == needle.charAt(0) && i + needle.length() <= array.length) {
                int k = i + 1;
                boolean flag = true;
                for (int j = 1; j < needle.length(); j++) {
                    if (k >= array.length || array[k++] != needle.charAt(j)) {
                        flag = false;
                        break;
                    }
                }
     ...
     */
//    作者：ji-zhi-de-pang-xi-fu
//    链接：https://leetcode-cn.com/problems/two-sum/solution/kmpsuan-fa-shi-xian-zi-fu-chuan-pi-pei-by-ji-zhi-d/
//    来源：力扣（LeetCode）
