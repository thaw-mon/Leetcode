...
/**
 * @题目 ：447. Number of Boomerangs
 * @Data 19/12/23
 * @题目描述： Given n points in the plane that are all pairwise distinct, a "boomerang" is a tuple of points (i, j, k) such that the distance between i and j equals the distance between i and k (the order of the tuple matters).
 * <p>
 * Find the number of boomerangs. You may assume that n will be at most 500 and coordinates of points are all in the range {-10000, 10000} (inclusive).
 * @题目链接： 链接：https://leetcode-cn.com/problems/number-of-boomerangs
 * @示例1: ######
 * Input:
 * {{0,0},{1,0},{2,0}}
 * <p>
...
public class NumberOfBoomerangs {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                for (int k = 0; k < n; k++) {
                    if (i == k || j == k) continue;
                    //计算i-j 和 i-k之间的距离
                    if (equalDistance(points[i], points[j], points[k]))
                        res++;
                }
            }
    ...
    //改为使用map记录不同节点间的距离:写复杂了 实际上只要一个map而不需要构建map数组
    public int numberOfBoomerangs2(int[][] points) {
        int n = points.length;
        System.out.println("n = " + n);
        int res = 0;
        Map<Long, Integer>[] maps = new Map[n];
        //maps{i}记录了 点i和其他节点距离的数量
        //初始化maps
        for (int i = 0; i < n; i++) {
            maps[i] = new HashMap<>();
        }
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                Long distance = (long)(points[i][0] - points[j][0]) * (points[i][0] - points[j][0]) + (long)(points[i][1] - points[j][1]) * (points[i][1] - points[j][1]);
                //点i和点j距离distance数量加1
                maps[i].put(distance, maps[i].getOrDefault(distance, 0) + 1);
                maps[j].put(distance, maps[j].getOrDefault(distance, 0) + 1);
            }
            //计算maps[i}中距离数量大于2的数目
            for (int value : maps[i].values()) {
                if (value >= 2)
                    res += value * (value-1); //数量A(2,N)
            }
        }
        return res;
    }
    ...
    private boolean equalDistance(int[] a, int[] b, int[] c) {
        return (int) Math.pow(a[0] - b[0], 2) + (int) Math.pow(a[1] - b[1], 2) == (int) Math.pow(a[0] - c[0], 2) + (int) Math.pow(a[1] - c[1], 2);
        }
}
